...
(через [Supadata](https://supadata.ai)) и анализ от нейросети
(через [OpenRouter](https://openrouter.ai)).

Доступные виды анализа:

- **Краткое summary** — сжатый пересказ видео;
...
Браузер ──> /api/analyze (сервер Next.js)
                 ...
                 │
                 └─2─> OpenRouter LLM (анализирует транскрипт по выбранному режиму)
                 │
                 <──── готовый анализ ──> показывается на сайте
```
...
lib/
  ...
  prompts.ts          промпты под 4 режима анализа
